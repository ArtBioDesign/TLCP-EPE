...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 直接使用输入embedding进行Attention，不需要BiGRU层
        
        self.attention = ScaledDotProductAttention(config.embed)  # 注意：这里使用embed维度而不是hidden_size*2
        
        self.fc1 = nn.Linear(config.embed, config.hidden_size2)  # 输入维度改为embed
    ...
    def forward(self, x):
        ...
        out = self.attention(x)  # [batch_size, embed]
